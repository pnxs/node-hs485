...
MockSerialPort.prototype.emitData = function(data) {
...
MockSerialPort.prototype.write = function(data, func) {
    ...
    if (this.expectedWrites.length === 0) {
        throw new assert.fail("no write expected (" + data + ")");
    }

   //console.log("check " + data + " agains: " + this.expectedWrites[0].data);
    if (JSON.stringify(data) === JSON.stringify(this.expectedWrites[0].data)) {
        ...
        console.log("unexpected write: " + data + " expect: " + this.expectedWrites[0].data);
    }
};
